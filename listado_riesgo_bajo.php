...
$objController = new lrb_controller($arrRolUser);
$objController->runAjax();
$objController->drawContentController();

class lrb_controller{
  ...
  public function __construct($arrRolUser){
    $this->objModel = new lrb_model($arrRolUser);
    $this->objView = new lrb_view($arrRolUser);
  ...
  function getListadoRiesgoBajo(){
    ...
    $strQuery = "SELECT prestamo.id, 
                   ...
                   FROM prestamo 
                        ...
                        INNER JOIN agencias
                                ON prestamo.agenciacodigo = agencias.codigo
                  WHERE prestamo.saldo_capital BETWEEN (SELECT rango_inicial FROM estado_premora WHERE id = 1) AND (SELECT rango_final FROM estado_premora WHERE id = 1) 
                        {$strAndDiasMora}
                        {$strAndAgenciaCodigo}
    ...
    if( !empty($result) ){
      while($row = mysqli_fetch_assoc($result)) {
        $arrListadoRB[$row["id"]]["NUMERO"] = $row["numero"];
        $arrListadoRB[$row["id"]]["AGENCIA"] = $row["nombreagencia"];
        $arrListadoRB[$row["id"]]["NOMBRES"] = $row["nombres"];
        $arrListadoRB[$row["id"]]["SALDO_CAPITAL"] = $row["saldo_capital"];
        $arrListadoRB[$row["id"]]["DIAS_MORA_CAPITAL"] = $row["dias_mora_capital"];
        $arrListadoRB[$row["id"]]["CAPITAL_DESEMBOLSADO"] = $row["capital_desembolsado"];
        $arrListadoRB[$row["id"]]["PROMESAS"] = $row["promesas"];
      }
    }
  ...
  public function drawBlurDetailContent($arrDetail){
    if( count($arrDetail)>0 ){
      ...
      <table class="table table-borderless table-sm">
        ...
        <!-- datos del prestamo-->
        <tr>
          <td colspan="4" style="background-color: #00a65a; color:white;"><h3>Datos del Préstamo</h3></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="3" style="background-color: #00a65a; color:white;"><h3>Gestión de pago</h3></td>
        </tr>
        <tr>
    ...
    if( count($arrListadoPromesas)>0 ){
      ...
      <table class="table table-sm table-borderless table-striped">
        ...
        while( $rTMP = each($arrListadoPromesas) ){
          ...
          ?>
          <tr>
            <td style="text-align:justify;  vertical-align:middle;"><h5><span class="badge bg-green"><?php print $intCount;?></span></h5></td>
  ...
  public function drawContent(){
    ...
    <div class="wrapper">
      ...
      <aside class="main-sidebar">
        ...
        <section class="sidebar">
          ...
          <?php draMenu($this->arrRolUser,'listado_riesgo_bajo', 1); ?>
        </section>
        <!-- /.sidebar -->
      </aside>
          ...
          <div class="row">
            <div class="box box-info">
              ...
              <div id="no-more-tables">
                  <table class="table table-sm table-hover table-condensed table-striped" id="tblListadoRB">
                      <thead class="cf">
                          <tr style="background-color: #00a65a; color:white;">
                              <th style="text-align:center;">No.</th>
                          ...
                          if( count($arrListadoRB)>0 ){
                              ...
                              while( $rTMP = each($arrListadoRB) ){
                                  ...
                                  <tr id="trId_<?php print $intId;?>">
                                      ...
                                      <td style="text-align:center;"><button class="btn btn-success btn-raised" title="Ver detalles" onclick="getDetailPrestamo('<?php print $intId?>')">Ver detalles</button></td>
                                  </tr>
                                  <?php
                              }
                          }
    ...
    <script>
            ...
            $.ajax({
              ...
              success:function(data){
                if ( data.Correcto == "Y" ){
                  ...
                  location.href = "listado_riesgo_bajo.php";
                }
              }
            });
          }
        ...
        $( document ).ready(function() {
          ...
          $('#tblListadoRB').DataTable({
            paging: false
          });
          
          $('#tblListadoRB_wrapper').find('label').each(function () {
              $(this).parent().append($(this).children());
          });
          $('#tblListadoRB_wrapper .dataTables_filter').find('input').each(function () {
              const $this = $(this);
              $this.removeClass('form-control-sm');
          });
          $('#tblListadoRB_wrapper .dataTables_length').addClass('d-flex flex-row');
          $('#tblListadoRB_wrapper .dataTables_filter').addClass('md-form');
          $('#tblListadoRB_wrapper select').removeClass('custom-select custom-select-sm form-control form-control-sm');
          $('#tblListadoRB_wrapper select').addClass('mdb-select');
          $('#tblListadoRB_wrapper .mdb-select').materialSelect();
          $('#tblListadoRB_wrapper .dataTables_filter').find('label').remove();
        });
