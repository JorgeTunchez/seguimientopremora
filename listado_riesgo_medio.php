...
$objController = new lrm_controller($arrRolUser);
$objController->runAjax();
$objController->drawContentController();

class lrm_controller{
  ...
  public function __construct($arrRolUser){
    $this->objModel = new lrm_model($arrRolUser);
    $this->objView = new lrm_view($arrRolUser);
...
class lrm_model{
  ...
  function getListadoRiesgoMedio(){
    $conn = getConexion();
    ...
    $strQuery = "SELECT prestamo.id, 
                   ...
                   FROM prestamo 
                        ...
                        INNER JOIN agencias
                                ON prestamo.agenciacodigo = agencias.codigo
                  WHERE prestamo.saldo_capital BETWEEN (SELECT rango_inicial FROM estado_premora WHERE id = 2) AND (SELECT rango_final FROM estado_premora WHERE id = 2) 
                        {$strAndDiasMora}
                        {$strAndAgenciaCodigo}
    ...
    if( !empty($result) ){
      while($row = mysqli_fetch_assoc($result)) {
        $arrListadoRM[$row["id"]]["NUMERO"] = $row["numero"];
        $arrListadoRM[$row["id"]]["AGENCIA"] = $row["nombreagencia"];
        $arrListadoRM[$row["id"]]["NOMBRES"] = $row["nombres"];
        $arrListadoRM[$row["id"]]["SALDO_CAPITAL"] = $row["saldo_capital"];
        $arrListadoRM[$row["id"]]["DIAS_MORA_CAPITAL"] = $row["dias_mora_capital"];
        $arrListadoRM[$row["id"]]["CAPITAL_DESEMBOLSADO"] = $row["capital_desembolsado"];
        $arrListadoRM[$row["id"]]["PROMESAS"] = $row["promesas"];
      }
    }
  ...
  public function drawBlurDetailContent($arrDetail){
    if( count($arrDetail)>0 ){
      ...
      <table class="table table-borderless table-sm">
        ...
        <!-- datos del prestamo-->
        <tr>
          <td colspan="4" style="background-color: #f39c12; color:white;"><h3>Datos del Préstamo</h3></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="3" style="background-color: #f39c12; color:white;"><h3>Gestión de pago</h3></td>
        </tr>
        <tr>
    ...
    if( count($arrListadoPromesas)>0 ){
      ...
      <table class="table table-sm table-borderless table-striped">
        ...
        while( $rTMP = each($arrListadoPromesas) ){
          ...
          ?>
          <tr>
            <td style="text-align:justify;  vertical-align:middle;"><h5><span class="badge bg-yellow"><?php print $intCount;?></span></h5></td>
  ...
  public function drawContent(){
    ...
    <div class="wrapper">
      ...
      <aside class="main-sidebar">
        ...
        <section class="sidebar">
          ...
          <?php draMenu($this->arrRolUser,'listado_riesgo_medio', 1); ?>
        </section>
        <!-- /.sidebar -->
      </aside>
          ...
          <div class="row">
            <div class="box box-info">
              ...
              <div id="no-more-tables">
                  <table class="table table-sm table-hover table-condensed table-striped" id="tblListadoRM">
                      <thead class="cf">
                          <tr style="background-color: #f39c12; color:white;">
                              <th style="text-align:center;">No.</th>
                              ...
                              while( $rTMP = each($arrListadoRM) ){
                                  ...
                                  <tr id="trId_<?php print $intId;?>">
                                      <td data-title="No." style="text-align:center; vertical-align:middle;">
                                        <h5><span class="badge bg-yellow"><?php print $intCount;?></span></h5>
                                      </td>
                                      ...
                                      <td style="text-align:center;"><button class="btn btn-warning btn-raised" title="Ver detalles" onclick="getDetailPrestamo('<?php print $intId?>')">Ver detalles</button></td>
                                  </tr>
                                  <?php
                              }
                          }
    ...
    <script>
            ...
            $.ajax({
              ...
              success:function(data){
                if ( data.Correcto == "Y" ){
                  ...
                  location.href = "listado_riesgo_medio.php";
                }
              }
            });
          }
        ...
        $( document ).ready(function() {
          ...
          $('#tblListadoRM').DataTable({
            paging: false
          });

          $('#tblListadoRM_wrapper').find('label').each(function () {
              $(this).parent().append($(this).children());
          });
          $('#tblListadoRM_wrapper .dataTables_filter').find('input').each(function () {
              const $this = $(this);
              $this.removeClass('form-control-sm');
          });
          $('#tblListadoRM_wrapper .dataTables_length').addClass('d-flex flex-row');
          $('#tblListadoRM_wrapper .dataTables_filter').addClass('md-form');
          $('#tblListadoRM_wrapper select').removeClass('custom-select custom-select-sm form-control form-control-sm');
          $('#tblListadoRM_wrapper select').addClass('mdb-select');
          $('#tblListadoRM_wrapper .mdb-select').materialSelect();
          $('#tblListadoRM_wrapper .dataTables_filter').find('label').remove();
        });
